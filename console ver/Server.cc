...
void Server::do_messages()
{
    while (true)
    {
		...
		if (m.type == Message::LOGOUT)
		{
			...
			if (gameStarted)
			{
				//si la partida ha empezado, dado que no contamos con una IA no queda otra que cerrar el servidor
				logoutInProgress = true;
				shutdown();
			}
			else
			{
				//se avisa a todos los jugadores de que se ha desconectado este jugador
				Message message = Message(m.id, Message::LOGGED_OUT, players[getPlayerByID(m.id)].n);
				send(message);
			}
